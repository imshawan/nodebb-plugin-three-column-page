...
plugin.init = async function (params) {
	...
	helpers.setupAdminPageRoute(app, '/admin/plugins/three-column-page', middleware, [], renderAdmin);
...
pubsub.on('three-column-page:save', async function (pages) {
	storeData(pages);
	await plugin.saveTemplates(pages);
});
...
async function getCustomPages() {
	...
	const data = await db.get('plugins:three-column-page');
...
plugin.addAdminNavigation = async function (header) {
	header.plugins.push({
		route: '/plugins/three-column-page',
		icon: 'fa-mobile',
...
plugin.saveTemplates = async function (pages) {
	...
	const customTPL = await fs.promises.readFile(path.join(__dirname, 'templates/three-column-page.tpl'), 'utf-8');
	try {
		...
		winston.error('[plugin/three-column-page] Could not save templates!');
